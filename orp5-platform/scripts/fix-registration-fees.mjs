...
<blockquote>
<p><strong>Note:</strong> Late fee of INR 1,000 or $20 will be charged per person after the registration deadline of 15 September 2026 (Registration may be kept open till 15 September 2026). The registration fee does not include accommodation charges.</p>
</blockquote>
...
async function fix() {
  ...
  // Remove fabricated fee sections and replace with correct ones
  // Find and replace from "Registration Fee" heading to the FAQ section
  const feeStart = content.indexOf('<h2>Registration Fee');
  const faqStart = content.indexOf('<h2>Frequently Asked Questions');

  if (feeStart > -1 && faqStart > -1) {
    content = content.substring(0, feeStart) + correctFeesHTML + '\n' + content.substring(faqStart);
  } else if (feeStart > -1) {
    content = content.substring(0, feeStart) + correctFeesHTML;
  }

  // Also remove fabricated cancellation policy
  content = content.replace(/<h2>Cancellation Policy<\/h2>[\s\S]*?<h2>/, '<h2>');
  ...
  if (error) {
    ...
    console.log('FIXED registration fees');
  }
}
